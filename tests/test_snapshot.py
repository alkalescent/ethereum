...
class TestSnapshot:
    ...
    def test_create_makes_snapshot_checking_age(self, mock_boto3, mocker):
        """Test _create logic when all snapshots are old."""
        snapshot = Snapshot()
        mocker.patch.object(snapshot, "is_older_than", return_value=True)
        mocker.patch.object(snapshot, "force_create", return_value={"SnapshotId": "new-snap"})
        mocker.patch.object(snapshot, "_put_param")

        result = snapshot._create([{"SnapshotId": "old"}])

        assert result == {"SnapshotId": "new-snap"}
        snapshot.force_create.assert_called_once()

    def test_create_skips_if_recent_exists(self, mock_boto3, mocker):
        """Test _create logic when a recent snapshot exists."""
        snapshot = Snapshot()
        # Returns False for at least one snapshot (meaning it's recent)
        ...
        assert result is None
        # force_create should NOT be called if we didn't mock it (or we can assert not called on the mock if we did)
        # Here we rely on result being None implies logic path

    def test_update_refreshes_instance(self, mock_boto3, mocker):
        """Test update method triggering instance refresh."""
        snapshot = Snapshot()
        
        # Mock dependencies
        mocker.patch.object(snapshot, "_get_snapshots", return_value=[{"SnapshotId": "snap-new", "StartTime": datetime.utcnow()}])
        mocker.patch.object(snapshot, "_get_param", return_value="snap-old")
        mocker.patch.object(snapshot, "_put_param")
        
        # Configure the already-mocked EC2 client
        ...
        # Configure the already-mocked ASG client
        ...
        needs_update = snapshot.update()
        
        assert needs_update is True
        snapshot.ec2.create_launch_template_version.assert_called_once()
        snapshot.auto.update_auto_scaling_group.assert_called_once()

    def test_instance_is_draining(self, mock_boto3, mocker):
        """Test instance_is_draining check."""
        snapshot = Snapshot()
        snapshot.instance_id = "i-123"
        
        # Configure the already-mocked ECS client
    ...
    def test_backup_flow(self, mock_boto3, mocker):
        """Test the main backup flow."""
        snapshot = Snapshot()
